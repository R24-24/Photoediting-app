...
import { ImageData, ToolType } from '../types';
...
interface EditorProps {
  ...
  onApplyEdit: (prompt: string, mask: string | null) => void;
  isLoading: boolean;
}
...
const Editor: React.FC<EditorProps> = ({ originalImage, onGenerate, onApplyEdit, isLoading }) => {
  ...
  const handleRemoveBackground = () => {
    const bgPrompt = "Remove the background, make the background transparent.";
    onApplyEdit(bgPrompt, null);
  };

  const handleMaskApply = (mask: string, maskPrompt: string) => {
    const fullPrompt = `${maskPrompt} in the masked area. The rest of the image should remain unchanged.`
    onApplyEdit(fullPrompt, mask);
    setActiveTool('select');
  };

  const isMasking = activeTool === 'magic-edit';

  return (
    <div className="bg-base-200 rounded-2xl shadow-lg p-6 flex flex-col lg:flex-row gap-6 border border-base-300 animate-fade-in">
        <Toolbox 
        ...
        <div className="flex-grow flex flex-col space-y-4">
            <div className="relative flex-grow rounded-lg overflow-hidden flex items-center justify-center bg-black/20 min-h-[300px]">
                {isMasking ? (
                  <MaskingEditor 
                    imageUrl={`data:image/png;base64,${originalImage.base64}`}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                placeholder="Describe a poster to generate..."
