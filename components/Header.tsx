...
const Header: React.FC<HeaderProps> = ({ onSignOut, showSignOut, tokens, timeUntilReset, onShowHistory }) => {
  ...
  return (
    <header className="bg-base-200/50 backdrop-blur-sm p-2 sm:p-4 shadow-md sticky top-0 z-10">
      <div className="container mx-auto flex justify-between items-center flex-wrap gap-y-2">
        <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight">{t('header.title')}</h1>
        </div>
        <div className="flex items-center gap-2 sm:gap-4">
            {showSignOut && (
                <div className="flex items-center gap-2 sm:gap-4 bg-base-300 py-2 px-3 sm:px-4 rounded-lg">
                    ...
                    {timeUntilReset && (
                        <>
                            <div className="w-px h-4 bg-base-100 hidden sm:block"></div>
                            <div className="hidden sm:flex items-center gap-2 text-sm" title="Time until next free token reset">
            ...
            <div className="relative">
                <select 
                    ...
                    className="bg-base-300 text-white font-semibold py-2 px-3 rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-brand-primary cursor-pointer"
                    defaultValue="en"
                >
            ...
            {showSignOut && (
                <button 
                    ...
                    className="flex items-center justify-center bg-base-300 hover:bg-red-600 text-white font-semibold py-2 px-3 sm:px-4 rounded-lg transition-all duration-300 transform hover:scale-105 sm:space-x-2"
                >
